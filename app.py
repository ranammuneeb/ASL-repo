...
if os.path.exists(WORDS_FILE):
    ...
    print("⚠️  Warning: words.txt not found. Suggestions will be empty.")
...
class VideoCamera:
    ...
    def get_suggestions(self):
        ...
        # Get close matches (up to 4 suggestions)
        matches = difflib.get_close_matches(last_word, WORD_LIST, n=4, cutoff=0.3)
        
        # Also include matches that start with the last word
...
def get_sentence():
    # Helper to get suggestions based on current sentence state
    # We can re-use the logic from VideoCamera or just put it here
    suggestions = []
    if predicted_sentence:
        ...
        if words:
            ...
            if last_word:
                ...
                # Combine (starts_with has priority for typing feels)
                suggestions = list(set(starts_with + matches))
                # Filter out the word itself if it's already complete
